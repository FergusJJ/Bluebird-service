...
struct UserTracksCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "tracks",
        abstract: "Sync Spotify data for all users"
    )
    ...
    public func run() async throws {
        ...
        guard !SharedConfig.spotifyClientSecret.isEmpty else {
            print("Error: SPOTIFY_CLIENT_SECRET environment variable not set or empty.")
            return
        }

        guard !SharedConfig.spotifyClientID.isEmpty else {
    ...
    private func processUserSongs(
        ...
        do {
            ...
            BluebirdService.prettyPrint(artistsToInsert, label: "Artists to Insert")
            BluebirdService.prettyPrint(tracksToInsert, label: "Tracks to Insert")
            BluebirdService.prettyPrint(albumsToInsert, label: "Albums to Insert")
            try await Database.insertUserSongPlays(
                client: client,
